/* ==================================================================
    ...
    const TIMESTAMP_DEFAULT = "Monday, 22/06/26 11:11 AM GMT +05:30";
    ...
    const state = {
      ...
      cardWidthMult:  1.0,
      cardHeightMult: 1.0,
      cardFont:       { family: 'DM Sans', weight: 200 },
    };
    ...
    inpTitle.value     = DEFAULT_LOCATION.title;
    inpAddress.value   = DEFAULT_LOCATION.address;
    ...
    let tileLayer = L.tileLayer(MAP_TILES[activeTileKey].url, { attribution: "" }).addTo(map);
    ...
    async function buildSatelliteMiniMap(lat, lng, zoom = 16) {
      const tileUrl = MAP_TILES[activeTileKey].url;
      const tileSize = 256;
      ...
      // Draw pin
        ...
        const mx = offscreen.width  / 2;
        const my = offscreen.height / 2;
        const r  = 44; // Base scale radius

        offCtx.fillStyle = "#ff4d4f"; // Red pin body
        offCtx.beginPath();
    ...
    function applyCardVars(el, refWidth, refHeight) {
      ...
      const fontTitle  = ref * (isPortrait ? 0.055 : 0.030) * hm;
      const fontSub    = ref * (isPortrait ? 0.042 : 0.023) * hm;
      const fontGoogle = ref * (isPortrait ? 0.052 : 0.028) * hm;
      ...
      const padTop    = ref * (isPortrait ? (10 / 500) : (12 / 900)) * hm;
      const padLeft   = ref * (isPortrait ? (5  / 500) : (5  / 900)) * wm;
      const vertPad   = padTop * 2.8;
      const rowGap    = ref * (isPortrait ? 0.010 : 0.008) * hm;
      const cardH     = titleRowH + 4 * subRowH + vertPad + 4 * rowGap;

      const cardBot = H * 0.025;
      // Keep card horizontally centered accounting for both width mult and scale
      const scale   = state.cardScale;
      const cardL   = (W - cardW * scale) / 2;
      ...
      s.setProperty("--card-scale",   `${state.cardScale}`);
      s.setProperty("--card-font-family", `'${state.cardFont.family}', sans-serif`);
      s.setProperty("--card-font-weight", `${state.cardFont.weight}`);
    }
    ...
    function updateOverlay() {
      ...
      ovTitle.textContent   = loc.title   || DEFAULT_LOCATION.title;
    ...
    async function refreshMiniMap() {
      ...
      try {
        const dataUrl = await buildSatelliteMiniMap(lat, lng, state.mapZoom);
    ...
    // Font picker — apply each button's font to itself for preview
    document.querySelectorAll(".font-btn").forEach(btn => {
      if (btn.dataset.style) btn.style.cssText += btn.dataset.style;
    });
    ...
    document.getElementById("font-grid").addEventListener("click", e => {
      ...
      state.cardFont = { family: btn.dataset.font, weight: parseInt(btn.dataset.weight) };
      // Apply immediately to preview wrapper
      previewWrapper.style.setProperty("--card-font-family", `'${state.cardFont.family}', sans-serif`);
      previewWrapper.style.setProperty("--card-font-weight", `${state.cardFont.weight}`);
    });

    // Map mode switcher
    document.getElementById("map-mode-grid").addEventListener("click", e => {
      const btn = e.target.closest(".map-mode-btn");
      if (!btn) return;
      const key = btn.dataset.mode;
      if (key === activeTileKey) return;
      activeTileKey = key;
      document.querySelectorAll(".map-mode-btn").forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      // Swap Leaflet tile layer
      map.removeLayer(tileLayer);
      tileLayer = L.tileLayer(MAP_TILES[key].url, { attribution: "" }).addTo(map);
      // Refresh minimap
      refreshMiniMap();
    });
